...
import { ClassType } from '@/redux/endpoints/teacher/class';
import { useLazyStudentGetListClassQuery } from '@/redux/endpoints/student/class';
...
const BasicStudentSelectClass: FC<PropsType> = ({ className, placeholder, onChange, value }) => {
  const { ref: boxInputRef, isComponentVisible, setIsComponentVisible } = useComponentVisible(false);

  const [trigger, { data: list, isFetching }] = useLazyStudentGetListClassQuery();
  ...
  useEffect(() => {
    if (list) {
      ...
      const listKeyword =
        ((list?.result as ClassType[])?.map((e: ClassType) => ({ value: e.id as number, label: e.name as string })) as {
          value: number;
          label: string;
        }[]) ||
        ([] as {
...
export default BasicStudentSelectClass;
